...
var LegalEntityName = $("#legalentityval");
...
function PageControls() {
    ...
    LegalEntityName.keyup(function(e) {
        ...
        if (GroupId.val().trim() != "" && GroupId.val().trim() > 0) {
            le_list = [];
            for (var i = 0; i < _legalEntities.length; i++) {
                if (_legalEntities[i].client_id == GroupId.val().trim()) {
                    le_list.push({
                        "legal_entity_id": _legalEntities[i].legal_entity_id,
                        "legal_entity_name": _legalEntities[i].legal_entity_name
                    });
                }
            }
            commonAutoComplete(
...
function onAutoCompleteSuccess(value_element, id_element, val) {
    ...
    if (current_id == "group-id") {
        clearElement([LegalEntityId, LegalEntityName, users, userId, FormName, FormId]);
    } else if (current_id == "legalentityid") {
        clearElement([users, userId, FormName, FormId]);
    } else if (current_id == "userid") {
        clearElement([FormName, FormId]);
    }
}
...
function validateMandatory() {
    ...
    if (GroupId.val().trim() == '' || GroupId.val().trim() == null) {
        ...
        is_valid = false;
    } else if (LegalEntityId.val() == '' || LegalEntityId.val() == null) {
...
function fetchData() {
    ...
    _form_id = FormId.val().trim();
    _client_id = GroupId.val().trim();
    _le_id = LegalEntityId.val().trim();
    ...
    displayLoader();
    console.log(_from_date, _to_date, _user_id, _form_id, _client_id, _le_id)
    mirror.getClientAuditTrail(
        _from_date, _to_date, parseInt(_user_id), parseInt(_form_id), parseInt(_client_id), parseInt(_le_id), _sno, _page_limit,
        function(error, response) {
...
function exportData() {
    ...
    mirror.getExportClientAuditTrail(
        _from_date, _to_date, parseInt(_user_id), parseInt(_form_id), parseInt(_client_id), parseInt(_le_id), true,
        function(error, response) {
        	hideLoader();
            if (error == null) {
                hideLoader();
...
function renderAuditData(audit_data) {
    ...
    $('.le-header').text(LegalEntityName.val().trim());

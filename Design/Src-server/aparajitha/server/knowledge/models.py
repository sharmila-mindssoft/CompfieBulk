...
class Statutory(object) :
    def __init__(self, statutoryId, name, levelId, parentIds, parentMappings=None) :
...
class Compliance(object) :
    def __init__(
            ...
            repeatsEvery, repeatsTypeId, duration, durationTypeId, isActive
        ):
        ...
        self.repeatsTypeId = repeatsTypeId
        self.repeatsEvery = repeatsEvery
        self.durationTypeId = durationTypeId
    ...
    def toStructure(self) :
        ...
        return {
            ...
            "duration_type_id": self.durationTypeId,
...
class StatutoryMapping(object) :
    def __init__(
        ...
        geographyIds, geographyMappings, approvalStatus, isActive
    ) :
        self.countryId = countryId
        self.countryName = countryName 
        self.domainId = domainId
        ...
        self.geographyMappings = geographyMappings
    ...
    def toReportStructure(self) :
        self.getData()
        return {
            "country_id": self.countryId,
            "domain_id": self.domainId,
            "industry_ids": self.industryIds,
            "industry_name": self.industryNames,
            "statutory_nature_id": self.statutoryNatureId,
            "statutory_nature_name": self.statutoryNatureName,
            "statutory_provision": self.statutoryMappings,
            "compliances": self.compliances,
            "geographies_ids": self.geographyIds,
            "geography_mappings": self.geographyMappings,
            "is_active": self.isActive
...
class StatutoryMappingApi(object):
    ...
    def getComplianceDurations(self):
        DH = DatabaseHandler.instance()
        rows = DH.getComplianceDuration()
        durationList = []
        for row in rows :
            duration = {}
            duration["duration_type_id"] = row[0]
            duration["duration_type"] = row[1]
            durationList.append(duration)
        return durationList

    def getComplianceRepeats(self):
        DH = DatabaseHandler.instance()
        rows = DH.getComplianceRepeat()
        repeatList = []
        for row in rows :
            repeat = {}
            repeat["repeat_type_id"] = row[0]
            repeat["repeat_type"] = row[1]
    ...
    def getStatutoryMappings(self) :
        ...
        for row in _statutoryMapList :
            ...
            statutoryMappings = [_statutoryMappings.get(x)[3] for x in statutoryIds ]
...
class StatutoryMappingReport(object) :
    ...
    def getReportFilters(self) :
        ...
        self.industryList = IndustryList().getIndustries()
        self.statutoryNatureList = StatutoryNatureList().getStatutoryNatures()
        self.geographies= GeographyAPI(self.request, self.userId).getGeographyList()
        return [
    ...
    def getReportData(self) :
        ...
        industryId = requestData["industry_id"]
        if industryId is None :
            industryId = '%'
        statutoryNatureId = requestData["statutory_nature_id"]
        if statutoryNatureId is None :
            statutoryNatureId = '%'
        geographyId = requestData["geography_id"]
        if geographyId is None :
            geographyId = '%'
        ...
        level1Mappings = self.frameReportData(countryId, domainId, self.lookupAndFrameMappingData(mappingData))
        statutory_mappings = {}
        if level1Id is None :
            statutory_mappings = level1Mappings
        else :
            statutory_mappings[level1Id] = level1Mappings[level1Id]

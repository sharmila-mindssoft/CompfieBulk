...
$("#temp_addstatutories").click(function() {
  ...
  if(last_statutory_id==0){
    $(".error-message").html("No Statutory is selected");
  }else if($.inArray(last_statutory_id, sm_statutoryids) >= 0){
    $(".error-message").html("This Statutory already added");
  }else{
...
$("#temp_addcompliance").click(function() {
  ...
  if(statutory_provision.length == 0){
    ...
    $(".error-message").html("Compliance Frequency Required");
     ...
     $(".error-message").html("Repeats Every Required");
  }else{
    $(".error-message").html("");
  ...
  load_compliance();
    ...
    $(".error-message").html("Duplicate Compliance");
  }
  }     
});
        ...
        if( geographyList[i]["parent_id"] == geographyids[j][0] && geographyList[i]["level_id"] == levelid && geographyList[i]["is_active"] == 1) {
         if(status == 'add'){
          ...
          if($('#'+combineid).attr('class') == 'list'+setlevelstage+' active'){
            $(".error-message").html("Remove Child First");
...
function validate_firsttab(){
  if(sm_countryid == ''){
    ...
    $(".error-message").html("Industry Required");
  }else if (sm_statutorynatureid == ''){
    $(".error-message").html("Statutory Nature Required");
  }else{
    $(".error-message").html("");
    return true;
  }
...
function validate_secondtab(){
  if (sm_statutoryids.length == 0){
    $(".error-message").html("Atleast one Staturory should be selected");
  }else{
    $(".error-message").html("");
    return true;
  }
...
function validate_thirdtab(){
  if (compliances.length == 0){
    $(".error-message").html("Atleast one Compliance should be selected");
  }else{
    $(".error-message").html("");
    return true;
  }
...
function validate_fourthtab(){
  if (sm_geographyids.length == 0){
    $(".error-message").html("Atleast one Location should be selected");
  }else{
    $(".error-message").html("");
    return true;
  }

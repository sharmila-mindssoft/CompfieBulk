...
def return_statutory_settings(data, client_id):
...
def update_statutory_settings(db, data, session_user, client_id):
    ...
    update_opted_status_in_knowledge(data)
...
def execute_bulk_insert(db, value_list) :
    ...
    db.on_duplicate_key_update(table, ",".join(column), value_list, update_column)
...
def update_opted_status_in_knowledge(data):
    try :
        db_con = Database(
...
def get_users_for_seating_units(db, session_user, client_id):
    where_condition = " AND t1.is_primary_admin = 0 AND t1.is_active = 1 AND t2.unit_id \
        IN \
    ...
    query = "SELECT distinct t1.user_id, t1.service_provider_id, t1.employee_name, \
        ...
        (select form_ids from tbl_user_groups where user_group_id = t1.user_group_id)form_ids\
        FROM tbl_users t1 \
...
def return_assign_compliance_data(result, applicable_units, total):
    ...
    for r in result:
        ...
        due_date, due_date_list, date_list = set_new_due_date(statutory_dates, r["repeats_type_id"], c_id)
...
def save_assigned_compliance(db, request, session_user, client_id):
    ...
    db.on_duplicate_key_update("tbl_assigned_compliances", ",".join(columns), value_list, update_column)
...
def get_statutory_wise_compliances(
    ...
    if frequency_name is not None:
        condition += "AND c.frequency_id = (SELECT frequency_id FROM tbl_compliance_frequency WHERE \
        frequency = %s)"
        condition_val.append(frequency_name)
    else:
        condition += "AND c.frequency_id in %s"
        condition_val.append(tuple([2, 3]))
    ...
    query = "SELECT ac.compliance_id, ac.statutory_dates, ac.due_date,\
        ...
        INNER JOIN tbl_compliance_frequency f ON (c.frequency_id = f.frequency_id) \
        INNER JOIN tbl_compliance_repeat_type rt ON (c.repeats_type_id = rt.repeat_type_id) \
...
def validate_before_save(
    ...
    if is_already_completed_compliance(
        db, string_to_datetime(due_date), compliance_id, unit_id, [string_to_datetime(due_date)]
    ):
        return False
    else:
        return True
...
def save_past_record(
    ...
    rows = db.get_data(
        tblAssignedCompliances, concur_approve_columns, condition, [compliance_id, unit_id]
    )
    concurred_by = 0
    approved_by = 0
    if rows:
    ...
    values = [
        compliance_history_id, unit_id, compliance_id, string_to_datetime(due_date).date(),
        completion_date,
...
def get_compliance_approval_count(db, session_user):
    ...
    condition = "1"
    concur_count = 0
    condition = "IFNULL(completed_on, 0) != 0 AND IFNULL(approve_status, 0) != 1 "
    if (is_two_levels_of_approval(db)):
        condition += " AND IFNULL(concurrence_status, 0) != 1 AND concurred_by = %s"
    else :
        condition += " AND approved_by = %s"

    approve_count = db.get_data(
        tblComplianceHistory, columns, condition, [session_user]
    )[0]["compliance"]
...
def get_compliance_approval_list(
    ...
    query = "SELECT \
        ...
        ifnull(concurred_by, -1), remarks, datediff(tch.due_date, completion_date ), \
        compliance_task, compliance_description, tc.frequency_id, \
        (SELECT frequency FROM tbl_compliance_frequency tcf WHERE tcf.frequency_id = tc.frequency_id ), \
        ...
        (SELECT concat(unit_code, '-', tu.unit_name) FROM tbl_units tu \
        ...
        (SELECT domain_name from tbl_domains td WHERE td.domain_id = tc.domain_id )\
    ...
    if is_two_levels:
        # concurrence_condition = " AND IF ( (concurred_by = %s AND concurrence_status = 1), 0, 1)"
        condition = " AND IFNULL(concurrence_status, 0) != 0  AND concurred_by = %s"
    else :
        condition = " AND approved_by = %s "
    ...
    for row in rows:
        ...
        concurred_by_id = None if int(row[8]) == -1 else int(row[8])
        approved_by_id = None if int(row[19]) == -1 else int(row[19])
        compliance_history_id = row[0]
        start_date = datetime_to_string(row[2])
        due_date = datetime_to_string(row[3])
        documents = download_urls if len(download_urls) > 0 else None
        file_names = file_name if len(file_name) > 0 else None
        completion_date = None if row[5] is None else datetime_to_string(row[5])
        completed_on = None if row[6] is None else datetime_to_string(row[6])
        next_due_date = None if row[7] is None else datetime_to_string(row[7])
        concurred_by = None if concurred_by_id is None else get_user_name_by_id(db, concurred_by_id, client_id)
        remarks = row[9]
        ...
        validity_date = None if row[18] is [None, "None", ""] else datetime_to_string(row[18])
        unit_name = row[20]
        date_list = []
...
def approve_compliance(
    ...
    query = '''
        ...
        (SELECT frequency_id FROM tbl_compliances tc WHERE tch.compliance_id = tc.compliance_id ),
...
def get_compliance_history_details(db, compliance_history_id):
    columns = [
        ...
        "approved_by",
        "(select compliance_task from tbl_compliances c where c.compliance_id = ch.compliance_id ) as compliance_name ",
        "(select document_name from tbl_compliances c where c.compliance_id = ch.compliance_id ) as doc_name", "due_date"
    ]
    condition = "compliance_history_id = %s "

    rows = db.get_data(tblComplianceHistory + " ch", columns, condition, [compliance_history_id])
    print rows
    if rows:
        return rows[0]
    else:
    ...
    if concurrence_id != -1 and is_two_levels_of_approval(db) is True:
        ...
        if approval_status == "Approved":
            ...
            save_compliance_notification(
                db, compliance_history_id, notification_text, "Compliance Approved",
                "ApprovedToConcur"
            )
        else:
            ...
            save_compliance_notification(
                db, compliance_history_id, notification_text, "Compliance Concurred",
                "Approve"
            )
...
def reject_compliance_approval(db, compliance_history_id, remarks, next_due_date, client_id):
    ...
    condition = "compliance_history_id = %s "
    rows = db.get_data(
        tblComplianceHistory + " ch", columns, condition, [compliance_history_id]
    )
    unit_id = rows[0]["unit_id"]
    compliance_id = rows[0]["compliance_id"]
    due_date = rows[0]["due_date"]
    ...
    notify_compliance_rejected(
        db, compliance_history_id, remarks, "RejectApproval", rows[0]["assignee_id"],
...
def notify_compliance_rejected(
    ...
    approver_email, approver_name = get_user_email_name(db, str(approver_id))
    concurrence_email, concurrence_name = (None, None)
    if concurrence_id not in [None, "None", "", "null", "Null", 0] and is_two_levels_of_approval(db):
        concurrence_email, concurrence_name = get_user_email_name(db, str(concurrence_id))
...
def reject_compliance_concurrence(db, compliance_history_id, remarks, next_due_date, client_id):
    ...
    db.update(tblComplianceHistory, columns, values, condition)
    notify_compliance_rejected(
        db, compliance_history_id, remarks, "RejectConcurrence", rows[0]["assignee_id"],
        rows[0]["concurrence_id"], rows[0]["approval_id"], rows[0]["compliance_name"],
        due_date
    )
    return True
...
def get_compliance_for_assignee(db, session_user, assignee, from_count, to_count):
    admin_id = get_admin_id(db)
    result = []
    user_qry = ""
    if session_user > 0 and session_user != admin_id :
    ...
    WHERE \
        t1.assignee = %s \
        and t1.is_active = 1  "
    order = "ORDER BY t3.unit_id , t2.statutory_mapping , t2.frequency_id, t4.due_date \
        limit %s, %s "
    param = [assignee]
    if user_qry != "" :
        q += user_qry
...
def reassign_compliance(db, request, session_user):
    ...
    for c in compliances :
        ...
        db.update(tblAssignedCompliances, update_assign_column, update_assign_val, where_qry)
...
def update_user_settings(db, new_units):
    for n in new_units :
        ...
        if len(new_domains) > 0 :
            ...
            #     value_list = [values]
            #     db.bulk_insert(tblUserDomains, domain_columns, value_list, client_id)
        ...
        if len(new_countries) > 0 :
            ...
            db.bulk_insert(tblUserCountries, country_columns, country_values_list)
            # if country_id not in user_countries :
            #     country_columns = ["user_id", "country_id"]
            #     values = (user_id, country_id)
            #     value_list = [values]
            #     db.bulk_insert(tblUserCountries, country_columns, value_list, client_id)

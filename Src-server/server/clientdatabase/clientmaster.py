...
def update_service_provider(db, service_provider, session_user, client_id):
    ...
    and service_provider_id = %s " % service_provider.service_provider_id
...
def get_user_details(db, client_id, session_user):
    ...
    return return_user_details(db, rows, client_id, unit_ids)
        ...
        if len(unit_ids_list) > 0:
            if len(user_unit_ids) > 0:
                if set(user_unit_ids) & set(unit_ids_list):
                    ...
                    continue
        countries = get_user_countries(db, user["user_id"])
        domains = get_user_domains(db, user["user_id"])
...
def save_user_countries(db, country_ids, user_id):
    ...
    for c_id in country_ids:
        country_values_list.append((user_id, c_id))

    res = db.bulk_insert(tblUserCountries, country_columns, country_values_list)
    if res is False :
...
def save_user(db, user, session_user, client_id):
    ...
    # Save user into in knowledge db
...
def update_user_status(db, user_id, is_active, emp_name, session_user, client_id):
    columns = [
...
def update_admin_status(db, user_id, is_admin, employee_name, session_user, client_id):
    ...
    UpdateUserStatus(is_admin, user_id, client_id, "admin")
...
def get_audit_trails(
    ...
    if not is_primary_admin(db, session_user) and not is_admin(db, session_user):
        ...
        if len(unit_ids) == 1 :
            unit_ids.append(0)
        query = "SELECT DISTINCT user_id FROM tbl_user_units where unit_id in %s"
        rows = db.select_all(query, [tuple(unit_ids)])
        user_ids = ""

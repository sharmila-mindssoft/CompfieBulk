...
def get_compliance_status(
    ...
    order = "GROUP BY month, year, T2.domain_id, %s \
        ORDER BY month desc, year desc, %s " % (group_by_name, group_by_name)
...
def get_trend_chart(db, country_ids, domain_ids, client_id):
    ...
    for country_wise_timeline in country_domain_timelines:
        ...
        for domain_wise_timeline in domain_wise_timelines:
            ...
            compliance_history_ids, client_statutory_ids, unit_ids = get_compliance_history_ids_for_trend_chart(
            ...
            if compliance_history_ids not in [None, "None", ""]:
                for index, dates in enumerate(start_end_dates):
                    ...
                    if len(rows) > 0:
                        ...
                        complied_compliances = row["complied"] if row["complied"] != None else 0
        ...
        for country_wise_timeline in country_domain_timelines:
            ...
            for domain_wise_timeline in domain_wise_timelines:
                ...
                for index, dates in enumerate(start_end_dates):
                    ...
                    compliance_history_ids = get_compliance_history_ids_for_trend_chart(
...
def get_trend_chart_drill_down(
    ...
    if filter_type == "Group":
        columns = "group_concat(DISTINCT unit_id) as unit_ids"
        ...
        columns = "group_concat(DISTINCT tcs.unit_id) as units"
    ...
    if rows:
        unit_ids = [int(x) for x in rows[0]["units"].split(",")] if rows[0]["units"] != None else []
    drill_down_data = []
    for unit_id in unit_ids:
        ...
        for domain_wise_timeline in domain_wise_timelines:
            ...
            compliance_columns = "group_concat(distinct compliance_id) as compliances"
            ...
            compliance_ids = compliance_rows[0]["compliances"]
...
def get_compliances_details_for_status_chart(db, request, session_user, client_id, from_count, to_count):
    ...
    result = compliance_details_query(
        db, request, chart_type, compliance_status, from_count, to_count, session_user
    )
...
def frame_compliance_details_query(db, chart_type, compliance_status, request, from_count, to_count, user_id):
    domain_ids = request.domain_ids
    filter_type = request.filter_type
    ...
    if compliance_status == "Inprogress" :
        where_qry = " AND ((IFNULL(T4.duration_type_id,0) = 2 AND T1.due_date >= now()) or (IFNULL(T4.duration_type_id, 0) != 2 AND T1.due_date >= CURDATE())) \
                AND IFNULL(T1.approve_status, 0) != 1"
    ...
    if chart_type == "not_complied":
        ...
        if not_complied_type == "Below 30":
            where_qry += " AND abs(datediff(date(now()), date(T1.due_date))) <= 30 "
        elif not_complied_type == "Below 60":
            where_qry += " AND abs(datediff(date(now()), date(T1.due_date))) BETWEEN 30 and 60 "
        elif not_complied_type == "Below 90":
            if ageing > 60 and ageing <= 90 :
                where_qry += " AND abs(datediff(date(now()), date(T1.due_date))) BETWEEN 60 and 90 "
        else :
            where_qry += " AND abs(datediff(date(now()), date(T1.due_date))) > 90 "
...
def compliance_details_query(
    ...
    rows = frame_compliance_details_query(db, chart_type, compliance_status, data, from_count, to_count, user_id)
...
def get_user_company_details(db, user_id, client_id=None):
    ...
    if unit_ids not in [None, "None", ""]:
        columns = "group_concat(distinct division_id) as div_id, group_concat(distinct legal_entity_id) as leg_id, \
        group_concat(distinct business_group_id) as b_id"
        unit_condition = "1"
        ...
        division_ids = rows[0]["div_id"]
        legal_entity_ids = rows[0]["leg_id"]
        business_group_ids = rows[0]["b_id"]
...
def notify_expiration(db):
    ...
    columns = [
        "notification_id", "notification_type_id", "notification_text",
...
def get_client_statutory_ids_and_unit_ids_for_trend_chart(
    ...
    columns = "group_concat(client_statutory_id) as statu_id, group_concat(unit_id) as units"
    ...
    client_statutory_ids = result[0]["statu_id"]
    unit_ids = result[0]["units"]
...
def get_dashboard_notification_counts(
    ...
    statutory_notification_rows = db.get_data(
        tblStatutoryNotificationStatus, statutory_column, statutory_condition
    )

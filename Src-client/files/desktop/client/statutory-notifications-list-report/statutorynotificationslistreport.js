...
function initialize() {
  ...
  client_mirror.getStatutoryNotificationsListFilters(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
function loadStatutoryNotificationsListreport(buttontype) {
  ...
  if (countries == '') {
    ...
    client_mirror.getStatutoryNotificationsListReport(countriesNameVal, domainNameVal, businessgroupid, legalentityid, divisionid, unitid, level1id, fromdate, todate, csv, function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);

...
function initialize() {
  ...
  client_mirror.getSettings(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
$('#submit').click(function () {
  ...
  if ($('input[name=2levels]:checked').val() == 'yes')
    twolevelapproval = true;
  ...
  if (validate()) {
    ...
    client_mirror.updateSettings(twolevelapproval, parseInt(assigneeReminderDays), parseInt(eReminderAdvance), parseInt(eReminder), function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);

...
function initialize() {
  ...
  client_mirror.getClientUsers(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
function getUserGroupName(userGroupId) {
  var usergroupname;
  if (userGroupId != null) {
    $.each(userGroupsList, function (key, value) {
      //usergroupname
...
$('#submit').click(function () {
  ...
  if (checkLength) {
    ...
    if (usertype == '') {
      ...
      arrayUnits = arrayUnits.filter(function (n) {
        return n != undefined;
      });
      ...
      client_mirror.saveClientUser(clientUserDetailDict, function (error, response) {
        if (error == null) {
          onSuccess(response);
        } else {
          onFailure(error);
      ...
      client_mirror.updateClientUser(clientUserDetailDict, function (error, response) {
        if (error == null) {
          onSuccess(response);
        } else {
          onFailure(error);
  ...
  $('.warning-confirm').dialog({
    ...
    buttons: {
      Ok: function () {
        ...
        client_mirror.changeClientUserStatus(userId, isActive, employeeCodeName, function (error, response) {
          if (error == null) {
            onSuccess(response);
          } else {
            onFailure(error);
  ...
  $('.warning-confirm').dialog({
    ...
    buttons: {
      Ok: function () {
        ...
        client_mirror.changeAdminStatus(userId, isAdmin, employeeName, function (error, response) {
          if (error == null) {
            onSuccess(response);
          } else {
            onFailure(error);
...
function onArrowKeyUser(e, ac_item) {
  ...
  if (e.keyCode == 40) {
    if (chosen_clientuser === '') {
      chosen_clientuser = 0;
    } else if (chosen_clientuser + 1 < $('#' + ac_item + ' li').length) {
      chosen_clientuser++;
    }
    $('#' + ac_item + ' li').removeClass('auto-selected');
    $('#' + ac_item + ' li:eq(' + chosen_clientuser + ')').addClass('auto-selected');
    return false;
  }
  if (e.keyCode == 38) {
    if (chosen_clientuser === '') {
      chosen_clientuser = 0;
    } else if (chosen_clientuser > 0) {
      chosen_clientuser--;
    }
    $('#' + ac_item + ' li').removeClass('auto-selected');
    $('#' + ac_item + ' li:eq(' + chosen_clientuser + ')').addClass('auto-selected');
...
$('#domainsselected').keyup(function (e) {
  onArrowKeyUser(e, 'ulist-domain');
});
$('#countryselected').keyup(function (e) {
  onArrowKeyUser(e, 'ulist-country');
});
$('#bgroupsselected').keyup(function (e) {
  onArrowKeyUser(e, 'ulist-bg');
});
$('#legal-entities-selected').keyup(function (e) {
  onArrowKeyUser(e, 'ulist-legalentity');
});
$('#division-selected').keyup(function (e) {
...
function unitview() {
  ...
  for (var i = 0; i < countryarray.length; i++) {
    arrayCountries[i] = parseInt(countryarray[i]);
  }
  if ($('#domains').val() != '') {

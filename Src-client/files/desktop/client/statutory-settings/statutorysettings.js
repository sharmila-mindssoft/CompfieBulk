...
function load_statutory(sList) {
  ...
  for (var statutory in sList) {
    ...
    if (compliance_applicable_status) {
      $('.applicable', clone2).html('<img src=\'/images/tick1bold.png\' title="Applicable"/> <input type="hidden" id="applicable' + statutoriesCount + '" value="' + compliance_applicable_status + '"> </input> ');
    } else {
      $('.applicable', clone2).html('<img src=\'/images/deletebold.png\' title="Not Applicable"/> <input type="hidden" id="applicable' + statutoriesCount + '" value="' + compliance_applicable_status + '"> </input>');
    }
  ...
  if (totalRecord <= statutoriesCount) {
    $('#pagination').hide();
    $('#submit').show();
    $('#cancel').show();  /*$(document).ready(function($) {
      $(".act-label").on("click", function(event){
        accordionstatus = false;
      });
...
$('#pagination').click(function () {
  ...
  displayLoader();
  client_mirror.getStatutorySettingsCompliance(unit_id, s_endCount, function (error, response) {
    if (error == null) {
...
function submit_statutory() {
  ...
  if (password == '') {
    ...
    client_mirror.updateStatutorySettings(password, uVal, parseInt(uId), assignedStatutories, function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);
...
function displayEdit(unit_id, dispBusinessGroup, dispLegalEntity, dispDivision, dispUnit, isClosed) {
  displayLoader();
  s_endCount = 0;
  client_mirror.getStatutorySettingsCompliance(unit_id, parseInt(s_endCount), function (error, response) {
    if (error == null) {
      sList = response.statutories;
      totalRecord = response.total_count;
...
function loadCountwiseStatutorySettings(assignedStatutoriesList) {
  ...
  for (var entity in assignedStatutoriesList) {
    ...
    $('.tbl_edit', clone).html('<img src=\'/images/icon-edit.png\' onclick="displayEdit(' + unit_id + ',\'' + bGroup + '\',\'' + lEntity + '\',\'' + dName + '\',\'' + uName + '\',' + isClosed + ')"/>');
...
function getStatutorySettings() {
  ...
  client_mirror.getStatutorySettings(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
$('.listfilter').keyup(function () {
  ...
  for (var entity in assignedStatutoriesList) {
    ...
    if (assignedStatutoriesList[entity].business_group_name != null)
      filter2val = assignedStatutoriesList[entity].business_group_name;
    var filter3val = assignedStatutoriesList[entity].legal_entity_name;
    var filter4val = '-';
    if (assignedStatutoriesList[entity].division_name != null)
      filter4val = assignedStatutoriesList[entity].division_name;

...
function loadEscalations(escalations) {
  ...
  for (var reminder in escalations) {
    ...
    if (assignee != null) {
      str += '<li id="notification' + notificationId + '" class="' + readStatus + '" onclick="changeStatus(' + notificationId + ',' + escalations[reminder].read_status + ')"> <p style="width:90%;text-align:left">' + notificationText + '</p> <span style=\'font-weight:bold;vertical-align:bottom\'>' + assigneesplit[0] + ' <abbr class=\'page-load\' title=\'' + assigneesplit[0] + '\'> <img src=\'images/icon-info-blue.png\' style=\'width:15px;height:15px\'> </abbr></span> </li>';
    } else {
      str += '<li id="notification' + notificationId + '" class="' + readStatus + '" onclick="changeStatus(' + notificationId + ',' + escalations[reminder].read_status + ')"> <p style="width:90%;text-align:left">' + notificationText + '</p> <span style=\'font-weight:bold;vertical-align:bottom\'>' + assigneesplit[0] + ' <abbr class=\'page-load\' title=\'' + assigneesplit[0] + '\'> <img src=\'images/icon-info-blue.png\' style=\'width:15px;height:15px\'> </abbr></span> </li>';
    }
  }
...
function get_escalations(sno) {
  ...
  client_mirror.getNotifications('Escalation', sno, function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
function initialize() {
  ...
  sno = 0;
  notificationDict = [];
  get_escalations(sno);  /*setInterval(function() {
      get_escalations();
  }, 10000);*/
}
$(function () {

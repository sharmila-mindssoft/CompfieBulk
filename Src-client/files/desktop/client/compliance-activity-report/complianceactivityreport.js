...
function initialize() {
  ...
  client_mirror.getComplianceActivityReportFilters(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
  ...
  if (countries == '') {
    ...
    function onSuccess(data) {
      ...
      if (buttontype == 'export') {
        ...
        window.open(download_url, '_blank');  // if (error == null){
                                              // }
                                              // else {
                                              //     displayMessage(error);
                                              // }
    ...
    displayLoader();
    client_mirror.getComplianceActivityReportData(usertype, userid, parseInt(countries), parseInt(domain), level1id, unitid, complianceid, fromdate, todate, csv, function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);
...
$('#complianceval').keyup(function (e) {
  var textval = $(this).val();
  getComplianceTaskAutocomplete(e, textval, complianceList, function (val) {

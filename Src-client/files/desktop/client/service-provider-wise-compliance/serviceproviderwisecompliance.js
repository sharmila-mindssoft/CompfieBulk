...
function getServiceProviderReportFilters() {
  ...
  client_mirror.getServiceProviderReportFilters(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
function loadresult(filterList) {
  ...
  for (var entity in filterList) {
    ...
    if (sp_name != null || sp_name != '') {
      ...
      var spAddress = '';
      if (filterList[entity].address != null)
        spAddress = filterList[entity].address;
      ...
      for (var compliancelist in compliancelists) {
        ...
        for (i = 0; i < compliancelists[compliancelist].length; i++) {
          ...
          for (j = 0; j < compliancelists[compliancelist][i].statutory_dates.length; j++) {
            var sDay = '';
            if (compliancelists[compliancelist][i].statutory_dates[j].statutory_date != null)
              sDay = compliancelists[compliancelist][i].statutory_dates[j].statutory_date;
            var sMonth = '';
            if (compliancelists[compliancelist][i].statutory_dates[j].statutory_month != null)
              sMonth = compliancelists[compliancelist][i].statutory_dates[j].statutory_month;
            var tDays = '';
            if (compliancelists[compliancelist][i].statutory_dates[j].trigger_before_days != null)
              tDays = compliancelists[compliancelist][i].statutory_dates[j].trigger_before_days;
            if (sMonth != '')
          ...
          var dDate = '-';
          if (compliancelists[compliancelist][i].due_date != null)
            dDate = compliancelists[compliancelist][i].due_date;
          $('.tbl_duedate', clone3).text(dDate);
          var vDate = '-';
          if (compliancelists[compliancelist][i].validity_date != null)
            vDate = compliancelists[compliancelist][i].validity_date;
...
function loadCompliance(csv) {
  ...
  if (country.length == 0) {
    ...
    client_mirror.getServiceProviderWiseCompliance(parseInt(country), parseInt(domain), act, parseInt(unit), 
      parseInt(serviceprovider), fromCount, perPage, csv, function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);
...
function fetchData(page) {
  fromCount = (page - 1 ) * perPage;
  client_mirror.getServiceProviderWiseCompliance(parseInt(country), parseInt(domain), act, parseInt(unit), 
    parseInt(serviceprovider), fromCount, perPage, false, function (error, response) {
      if (error == null) {
        serviceProviderWiseComplianceList = response.compliance_list;
        totalRecord = response.total_count;
        showPaginationView(page);
        hideLoader();
      } else {
        displayMessage(error);
        hideLoader();
...
//pagination process
function showPaginationView(page) {
  cPage = parseInt(page);
  displayLoader();
  sno = (page - 1 ) * perPage;
  fromCount = sno;
...
$('#serviceproviderval').keyup(function () {
  ...
  if (textval.length > 0) {
    for (var i in serviceprovider) {
      if (~serviceprovider[i].service_provider_name.toLowerCase().indexOf(textval.toLowerCase()))
        suggestions.push([
          serviceprovider[i].service_provider_id,
          serviceprovider[i].service_provider_name
        ]);
    }
    var str = '';
    ...
    $('#ulist_serviceprovider').append(str);  //$("#serviceprovider").val('');
  } else {
    $('#serviceprovider').val('');
    $('#autocomplete_serviceprovider').hide();
  }

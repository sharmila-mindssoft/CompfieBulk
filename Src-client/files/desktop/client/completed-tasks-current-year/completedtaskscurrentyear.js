...
function load_thirdwizard() {
  ...
  $('.breadcrumbs').html($('.countrylist.active').text() + arrowimage + $('.legalentitylist.active').text() + arrowimage + $('.unitlist.active').text() + arrowimage + $('.domainlist.active').text());
      ...
      $('#assigneeval' + sno).keyup(function () {
        ...
        if (textval.length > 0) {
          for (var i in assignees) {
            if (~assignees[i].employee_name.toLowerCase().indexOf(textval.toLowerCase()))
              suggestions.push([
                assignees[i].user_id,
                assignees[i].employee_name
              ]);
          }
          var str = '';
...
function submitcompliance() {
  ...
  for (var i = 1; i <= sno; i++) {
    ...
    if (complianceApplicable) {
      ...
      if (completed_by != '')
        completed_by = parseInt(completed_by);
      if (due_date == '') {
        ...
        if (parseMyDate(due_date) > parseMyDate(validity_date)) {
          ...
          hideLoader();
          return false;  /* }else if(dateDifference > 90){
  ...
  client_mirror.savePastRecords(compliance_list, function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
...
function getStatutories() {
  ...
  if ($('.countrylist.active').attr('id') != undefined)
    assignComplianceCountryId = parseInt($('.countrylist.active').attr('id'));
  if ($('.unitlist.active').attr('id') != undefined)
    assignComplianceUnitId = parseInt($('.unitlist.active').attr('id'));
  if ($('.domainlist.active').attr('id') != undefined)
    assignComplianceDomainId = parseInt($('.domainlist.active').attr('id'));
  if ($('.actlist.active').attr('id') != undefined)
    assignComplianceActId = $('.actlist.active').attr('id');
  if ($('.frequencylist.active').attr('id') != undefined)
    assignComplianceFrequencyId = $('.frequencylist.active').attr('id');
  if (assignComplianceUnitId != null && assignComplianceDomainId != null) {
    ...
    client_mirror.getStatutoriesByUnit(assignComplianceUnitId, assignComplianceDomainId, assignComplianceActId, assignComplianceFrequencyId, assignComplianceCountryId, sno, function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);
  ...
  if (assignStatutoryLegalEntityId != null) {
    ...
    for (var industryunit in unitsList) {
      ...
      for (var unit in iUnits) {
        if (iUnits[unit].business_group_id == assignStatutoryBusinessGroupId && iUnits[unit].division_id == assignStatutoryDivisionId && iUnits[unit].legal_entity_id == assignStatutoryLegalEntityId && iUnits[unit].country_id == assignStatutoryCountryId) {
          str += splittext + '<li id="' + iUnits[unit].unit_id + '" class="unitlist" > <abbr class="page-load" title="' + iUnits[unit].address + '"><img src="/images/icon-info.png" style="margin-right:10px"></abbr>' + iUnits[unit].unit_name + '</li>';
          splittext = '';
        }
      }
    }
...
function getPastRecords() {
  ...
  client_mirror.getPastRecordsFormData(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);

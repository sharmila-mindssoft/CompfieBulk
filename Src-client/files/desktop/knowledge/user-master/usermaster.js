...
function displayEdit(userId) {
  ...
  for (var entity in usersList) {
    if (usersList[entity].user_id == userId) {
      ...
      var address = '';
      if (usersList[entity].address != null && usersList[entity].address != 'None')
        address = usersList[entity].address;
      ...
      var userGroupval;
      var designation = '';
      if (usersList[entity].designation != null && usersList[entity].designation != 'None')
  ...
  if (validate()) {
    if ($('#userid').val() == '') {
      ...
      userDetail = [
        ...
        employeeName,
        employeeId,
        countryCode + '-' + areaCode + '-' + contactNo,
        address,
        designation,
        countryIds,
        domainIds
      ...
      mirror.saveAdminUser(userDetailDict, function (error, response) {
        if (error == null) {
          onSuccess(response);
        } else {
          onFailure(error);
      ...
      mirror.updateAdminUser(userDetailDict, function (error, response) {
        if (error == null) {
          onSuccess(response);
        } else {
          onFailure(error);

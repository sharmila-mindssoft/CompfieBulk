...
$('#submit').click(function () {
  ...
  if (validate()) {
    if (industryId == '') {
      ...
      mirror.saveIndustry(industryName, function (error, response) {
        if (error == null) {
          onSuccess(response);
        } else {
          onFailure(error);
      ...
      mirror.updateIndustry(parseInt(industryId), industryName, function (error, response) {
        if (error == null) {
          onSuccess(response);
        } else {
          onFailure(error);
...
$('#search-industry-name').keyup(function () {
  ...
  for (var entity in industriesList) {
    ...
    if (~industryName.toLowerCase().indexOf(filterkey))
      filteredList.push(industriesList[entity]);
  }

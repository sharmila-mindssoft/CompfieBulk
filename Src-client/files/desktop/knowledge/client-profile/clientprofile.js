...
function initialize() {
  ...
  mirror.getClientProfile(function (error, response) {
    if (error == null) {
      onSuccess(response);
    } else {
      onFailure(error);
  ...
  $('.warning-confirm').dialog({
    ...
    buttons: {
      Ok: function () {
        ...
        mirror.changeClientUserStatus(userId, status, function (error, response) {
          if (error == null) {
            onSuccess(response);
          } else {
            onFailure(error);
  ...
  $('.warning-confirm').dialog({
    ...
    buttons: {
      Ok: function () {
        ...
        mirror.createNewAdmin(userId, clientId, currentAdminId, userName, function (error, response) {
          if (error == null) {
            onSuccess(response);
          } else {
            onFailure(error);

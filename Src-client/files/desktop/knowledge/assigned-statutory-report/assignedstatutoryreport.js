...
$('#show-button').click(function () {
  ...
  if (countries == '') {
    ...
    mirror.getAssignedStatutoryReport(parseInt(countries), domainsVal, groupid, businessgroupid, lentityid, divisionid, unitid, level1Statutoryid, applicableStatus, function (error, response) {
      if (error == null) {
        onSuccess(response);
      } else {
        onFailure(error);
...
$('#level1val').keyup(function (e) {
  var textval = $(this).val();
  getStatutoryAutocomplete(e, textval, level1List[$('#countries').val()][$('#domain').val()], function (val) {

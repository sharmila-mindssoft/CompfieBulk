  ...
  $('.warning-confirm').dialog({
    ...
    buttons: {
      Ok: function () {
        ...
        mirror.changeClientGroupStatus(parseInt(clientId), isActive, function (error, response) {
          if (error == null) {
            onSuccess(response);
          } else {
            onFailure(error);
...
function getClientCountriesNameunion(countryid) {
  ...
  ccnew = [];
  var arrayCountries = [];
  for (var i = 0; i < editcountryval.length; i++) {
    arrayCountries[i] = parseInt(editcountryval[i]);
  }

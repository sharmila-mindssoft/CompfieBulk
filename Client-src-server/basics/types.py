...
class NamedType(Type) :
    def __init__(self, name) :
        msg = "name must starts with [a-z][A-Z]: %s" % (name,)
        assert name[0].isalpha(), msg
        self._name = name
...
class RecordType(NamedType) :
    def __init__(self, name, fields, base_class=None) :
        ...
        msg = "duplicate field name found in %s" % (
            name
        )
        assert len(self._field_names) == len(Set(self._field_names)), msg

...
CREATE TRIGGER `after_tbl_client_groups_insert` AFTER INSERT ON `tbl_client_groups`
 FOR EACH ROW BEGIN
   ...
   INSERT INTO tbl_audit_log(action,
    ...
    INSERT INTO tbl_client_replication_status (client_id, is_new_data, is_group) VALUES(NEW.client_id, 1, 1);
END
//
DELIMITER ;
...
CREATE TRIGGER `after_tbl_units_update` AFTER UPDATE ON `tbl_units`
 FOR EACH ROW BEGIN
   ...
   INSERT INTO tbl_audit_log(action,
                             ...
                             tbl_name)
  SELECT @action, NEW.client_id,
  NEW.legal_entity_id, NEW.unit_id, 'geography', parent_names, 'tbl_units'  FROM tbl_geographies WHERE geography_id=NEW.geography_id;
   END IF;

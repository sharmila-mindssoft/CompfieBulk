    ...
    client_mirror.getUnitWiseReport(
        ...
        if (error == null) {
            ...
            if (response.unit_compliances.length == 0) {
                ...
                t_this._total_record = response.unit_compliances.length;
                if (t_this._sno == 0) {
...
UnitWiseReport.prototype.hidePageView = function() {
    $('#pagination-rpt').empty();
    $('#pagination-rpt').removeData('twbs-pagination');
    $('#pagination-rpt').unbind('page');
};

UnitWiseReport.prototype.createPageView = function(a_obj, total_records) {
...
UnitWiseReport.prototype.showPagePan = function(showFrom, showTo, total) {
    var showText = 'Showing ' + showFrom + ' to ' + showTo +  ' of ' + total + ' entries ';
    $('.compliance_count').text(showText);
    $('.pagination-view').show();
};
UnitWiseReport.prototype.hidePagePan = function() {
    $('.compliance_count').text('');
    $('.pagination-view').hide();
}

UnitWiseReport.prototype.renderPageControls = function(e) {
    var t_this = this;
